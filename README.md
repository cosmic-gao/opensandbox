...
> 原理:deepagents 的异步文件工具会调用 `als`/`aread`/`aexecute` 等;`BaseSandbox` 又把这些异步文件操作都转发到 `self.aexecute`/`self.aupload_files`。因此 `AsyncOpenSandboxBackend` 只需原生实现 `aexecute`/`aupload_files`/`adownload_files`,整个异步接口便都是原生异步。它是 async-only:同步原语会抛 `NotImplementedError`。
...
`AsyncOpenSandboxBackend` 接口与之对应,但为异步:`await create(...)` / `await connect(...)`、`aexecute` / `aupload_files` / `adownload_files`、`aclose()` / `async with`;派生的 `als`/`aread`/`awrite`/`aedit`/`aglob`/`agrep` 均为原生异步。同步原语(`execute` 等)会抛 `NotImplementedError`。
